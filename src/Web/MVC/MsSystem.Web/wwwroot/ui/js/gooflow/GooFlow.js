...
//初始化区域图的对象
	...
	this.$id=this.$bgDiv.attr("id")||'GooFlow_'+createGuid();
	...
	this.$bgDiv.append("<div class='GooFlow_work' style='"+(property.haveHead? "top:30px;":"")+(property.haveTool? "left:35px":"")+"'></div>");
	...
	_initGroup:function(width,height){
		...
		this.$group.mouseup({inthis:this},function(e){
			...
			if(e.data.inthis.$ghost.css("display")==="none"){
				...
				e.data.inthis.addArea(createGuid(),
	...
	_initWorkForNode:function(){
		...
		this.$workArea.on("mouseup",".GooFlow_item",{inthis:this},function(e){
			...
			if(lineStart&&!This.$mpTo.data("p")){
				...
				This.addLine(createGuid().toString(),tmp);
				This.$max++;
			}
			else{
				if(lineStart){
